...
public class WeightedMemoryStoreRouter : IMemoryStoreRouter
{
    ...
    public async Task<IEnumerable<MemoryStoreRoutingResult>> RankStoresAsync(string query, IEnumerable<MemoryStore> availableStores)
    {
        ...
        var rankingTasks = storeChunks.Select(async c => await RankStoreChunk(query, c));
        var result = await Task.WhenAll(rankingTasks);

        var flattenedResult = result.SelectMany(r => r);
        flattenedResult = flattenedResult.OrderByDescending(r => r.Relevance).ToList();

        return flattenedResult;
    }

    private async Task<List<MemoryStoreRoutingResult>> RankStoreChunk(string query, IEnumerable<MemoryStore> chunk, CancellationToken token = default)
    {
    ...
    private async Task<IEnumerable<MemoryStoreRoutingResult>> RankMemoryStoreChunk(Memory memory, IEnumerable<MemoryStore> chunk, CancellationToken token = default)
    {
        List<MemoryStoreRoutingResult> ranking = new();
        StringBuilder availableStoresStringBuilder = new();
        foreach (var store in chunk)
